...
"""
 CHOOSE YOUR CIRCUIT MODEL ANSATZ AND TRAIN/TEST DATA BELOW
"""



model, qubits, thetas = blocks.peps_8_online_full_a() # NB CHOOSE YOUR CIRCUIT MODEL ANSATZ!
data = genfromtxt('data/sudoku_shuffled_train.csv',delimiter=',')  # Training data
test_data = genfromtxt('data/sudoku_shuffled_test.csv',delimiter=',')  # Test data
...
train_step = tf.train.AdamOptimizer(1e-3,0.9).minimize(error)
...
thetas_np[22]=thetas[11][0].eval()
thetas_np[23]=thetas[11][1].eval()
thetas_np[24]=thetas[11][2].eval()
thetas_np[25]=thetas[11][3].eval()
"""
np.save('thetas_np_auto_8_QUBIT.npy',thetas_np)


np.save('x_test_auto_8_0_1.npy',x_test)
np.save('y_test_auto_8_0_1.npy',y_test)"""
print(thetas_np)
sess.close()